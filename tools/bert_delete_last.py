...
for epoch in range(0, epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        print(f"Batch {step + 1} - Taille des tenseurs:")
        ...
        print("predicted_classe = ",predicted_classes.long())
        print(" b_labels = ", b_labels.long())
    ...
    if epochs_without_improvement >= early_stopping_patience:
        print(f"Arrêt anticipé après {epoch+1} époques sans amélioration.")
        break

print("ts = ", training_stats)
"""
...
for epoch in range(0, epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # Afficher la taille des tenseurs dans chaque batch
        print(f"Batch {step + 1} - Taille des tenseurs:")
        #print(f"  Input ID: {input_id.size()}")
        #print(f"  Attention Mask: {attention_mask.size()}")
        #print(f"  Sentiment: {sentiment.size()}")
        ...
        logits = model(input_id, token_type_ids=None, attention_mask=attention_mask).logits
        #print("logits = ", logits)
        # Appliquer la fonction softmax pour obtenir les probabilités
        probabilities = F.softmax(logits, dim=1)
        # Trouver la classe prédite pour chaque exemple dans le batch
        output = torch.argmax(probabilities, dim=1)
        #logits = torch.argmax(logits, dim=1).float().requires_grad_()
        print("output = ", output, " sentiment = ", sentiment)
    ...
    for batch in val_dataloader:
        ...
        with torch.no_grad():  # Désactive le calcul du gradient pendant la validation
            # Récupération de la loss et des logits
            # On passe la donnee au model et on recupere la loss et le logits (sortie avant fonction d'activation)
            logits = model(input_id, token_type_ids=None, attention_mask=attention_mask).logits
            #print("logits = ", logits)
            # Appliquez la fonction argmax pour obtenir l'indice de la classe avec la probabilité la plus élevée
