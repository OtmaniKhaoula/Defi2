...
# Récupérer les 10 groupes de mots les plus fréquents
def frequence(corpus):
    textes_entier = ""
    for key in corpus.keys():
        texts = corpus[key]
        textes_entier = textes_entier + " " + texts
    list_words = word_tokenize(textes_entier,language="french",preserve_line=True)
    series_freq_words = pd.Series(list_words).value_counts()[0:10]
    dict_freq_words = series_freq_words.to_dict()
