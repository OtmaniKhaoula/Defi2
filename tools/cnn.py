...
data_train = data_train.iloc[:15000, :]
...
"""
# Utilisation de fastText pour le plongement
print("path = ", path)
model = fasttext.train_supervised(input=f"{path}/processed_data/train/data.tsv")
#model.test("processed_data/dev/data.tsv")

rev_comments_test = np.load(f"{path}/processed_data/test/comments.npy", allow_pickle=True).item()
rev_comments_train = np.load(f"{path}/processed_data/train/comments.npy", allow_pickle=True).item()

results_train = []

for key in tqdm.tqdm(rev_comments_train):
    # Remplacer la variable de type NoneType par une chaîne vide
    rev_comments_train[key] = "" if rev_comments_train[key] is None else rev_comments_train[key]
    comments = rev_comments_train[key].replace("\n"," ")
    embedding = model.get_sentence_vector(comments)
    results_train.append([key,embedding])

results_test = []

for key in tqdm.tqdm(rev_comments_test):
    rev_comments_test[key] = "" if rev_comments_test[key] is None else rev_comments_test[key]
    comments = rev_comments_test[key].replace("\n"," ")
    embedding = model.get_sentence_vector(comments)
    results_test.append([key,embedding])

rev_comments_dev = np.load(f"{path}/processed_data/dev/comments.npy", allow_pickle=True).item()

results_dev = []

for key in tqdm.tqdm(rev_comments_dev):
    rev_comments_dev[key] = "" if rev_comments_dev[key] is None else rev_comments_dev[key]
    comments = rev_comments_dev[key].replace("\n"," ")
    embedding = model.get_sentence_vector(comments)
    results_dev.append([key,embedding])

print("un exemple = ", results_dev[0])
#f = open(f'{path}/processed_data/embedding.txt', 'w', newline='')
...
def train(model, optimizer, train_dataloader, dev_dataloader=None, epochs=10, model_name=""):
    """Train the CNN model."""
